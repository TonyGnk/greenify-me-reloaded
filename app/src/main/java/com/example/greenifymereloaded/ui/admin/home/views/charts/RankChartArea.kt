...
@Composable
fun RankChartArea(
    producer: CartesianChartModelProducer, winnersItemList: List<Pair<String, Int>>,
) {
    val names = winnersItemList.map { it.first }.toMutableList()
    val points = winnersItemList.map { it.second }.toMutableList()
...
private fun Labels(
    ...
    val restSize = maxOf(0f, 100 - percentFirstToCurrent + 0.001f)
    Column(
        horizontalAlignment = Alignment.CenterHorizontally,
    ) {
        Column(
