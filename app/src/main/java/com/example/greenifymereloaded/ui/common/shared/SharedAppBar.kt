        ...
        modifier = modifier
            ...
            .padding(
                start = when (backBehavior is SharedBackBehavior.Enable) {
                    true -> dimensionResource(R.dimen.horizontalScreenPaddingSmall)
                    false -> 0.dp
                },
                end = if (backBehavior is SharedBackBehavior.Enable) dimensionResource(R.dimen.horizontalScreenPadding) else 0.dp,
            ),
    ...
    )
    {
        if (backBehavior is SharedBackBehavior.Enable)
            IconButton(onClick = { backBehavior.onClick() }) {
                Icon(
                    painterResource(R.drawable.angle_left),
